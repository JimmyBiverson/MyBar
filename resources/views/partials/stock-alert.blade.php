...
<ul class="dropdown-menu dropdown-menu-end p-0 stock-alert-menu" style="width:320px; max-width:90vw;">
    <li class="dropdown-header d-flex justify-content-between align-items-center px-3 py-2" style="font-weight:600; background:#f8f9fa; border-radius:8px 8px 0 0;">
        <span><i class="fas fa-exclamation-triangle text-warning me-1"></i> Low Stock Alerts</span>
        @if($lowStockCount > 0)
    ...
    @if($lowStockCount > 5)
        <li>
            <a class="dropdown-item text-center small py-2 text-primary fw-semibold" href="{{ route('products.index', ['low_stock' => 1]) }}">
                View all ({{ $lowStockCount }}) <i class="fas fa-arrow-right ms-1"></i>
            </a>
        </li>
    @endif
</ul>
