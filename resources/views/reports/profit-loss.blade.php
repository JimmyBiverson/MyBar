        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4 col-6">
                <div class="stat-box text-center" style="background:#e8f5e9">
                    ...
                    <h4 class="mb-0 text-success">{{ formatCurrency($data['total_revenue'] ?? 0) }}</h4>
                </div>
            </div>
            <div class="col-md-4 col-6">
            ...
            <div class="col-md-6">
                ...
                <div class="table-responsive">
                    <table class="table table-sm">
                        ...
                        <tbody>
                            <tr><td>Product Sales</td><td class="text-end">{{ formatCurrency($data['product_sales'] ?? 0) }}</td><td class="text-end">{{ $data['product_sales_percent'] ?? 0 }}%</td></tr>
                            <tr><td>Service Charges</td><td class="text-end">{{ formatCurrency($data['service_charges'] ?? 0) }}</td><td class="text-end">{{ $data['service_charges_percent'] ?? 0 }}%</td></tr>
                            <tr class="fw-bold"><td>Total Revenue</td><td class="text-end text-success">{{ formatCurrency($data['total_revenue'] ?? 0) }}</td><td class="text-end">100%</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-sm">
                        ...
                        <tbody>
                            ...
                            <tr><td>{{ $cat['name'] ?? 'N/A' }}</td><td class="text-end text-danger">{{ formatCurrency($cat['total'] ?? 0) }}</td></tr>
                            @empty
                            <tr><td colspan="2" class="text-center text-muted py-2">No expenses recorded</td></tr>
                            @endforelse
                            <tr class="fw-bold"><td>Total Expenses</td><td class="text-end text-danger">{{ formatCurrency($data['total_expenses'] ?? 0) }}</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
